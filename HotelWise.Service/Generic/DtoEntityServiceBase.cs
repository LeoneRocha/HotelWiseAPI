...
using SmartCoreHub.Core.SDK.Domain.Helpers;
...
/// <summary>
/// Base DTO-first local (PR-D6) que injeta o repositório canônico Infra IGenericRepository
/// (LongEntityBase) e implementa <see cref="IGenericDtoService{TDto}"/>.
/// </summary>
public abstract class DtoEntityServiceBase<T, TDto> : IGenericDtoService<TDto>
    where T : LongEntityBase, new()
    where TDto : class, new()
{
    protected readonly SmartCoreHub.Core.SDK.Infrastructure.Repositories.Generic.IGenericRepository<T> _repository;
    ...
    protected DtoEntityServiceBase(
        SmartCoreHub.Core.SDK.Infrastructure.Repositories.Generic.IGenericRepository<T> repository,
        IMapper mapper,
    ...
    public virtual async Task<ServiceResponse<TDto>> Validate(T item)
    {
        ...
        try
        {
            ...
            {
                List<ErrorResponse> errosTranslated = new();
                foreach (var errosItem in response.Errors)
                {
                    errosTranslated.Add(new ErrorResponse
                    {
                        Name = errosItem.Name,
                        Message = errosItem.DefaultMessage ?? string.Empty,
        ...
        catch (Exception ex)
        {
            ...
            response.Errors.Add(new ErrorResponse
            {
                Message = ex.Message,
                Code = "ValidateError",
                CodeType = ErrorCodeType.InternalError
            });
