...
/// <summary>
/// Validador FluentValidation para <see cref="ChatSessionHistory"/>, validando integridade de identificadores GUID, histórico de mensagens e totais de tokens.
/// </summary>
public class ChatSessionHistoryValidator : AbstractValidator<ChatSessionHistory>
{
    /// <summary>
    /// Configura as regras de validação para os dados da sessão de chat.
    /// </summary>
    ...
    /// <summary>
    /// Verifica se a string informada corresponde a um GUID sintaticamente válido.
    /// </summary>
    /// <param name="idToken">String do token a ser verificada.</param>
    /// <returns><c>true</c> se for um GUID válido; caso contrário, <c>false</c>.</returns>
    private static bool BeAValidGuid(string idToken)
    {
        return Guid.TryParse(idToken, out _);
    }
}
